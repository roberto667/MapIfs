...
@onready var voltar = $"CanvasLayer/Mapa completo"
@onready var escolher = $CanvasLayer/EscolhaLocal
@onready var show = $CanvasLayer/Referencia
@onready var mapacompleto = $Camera2D/Mapacompleto
@onready var fundomapacompleto = $Camera2D/Fundomapacompleto
var ver = false
var loca1
var loca2

func _ready():
	locais = get_node("Camera2D/escola/floor/locais")
...
func _process(delta):
	loca1 = get_node("Camera2D/escola/floor/locais/"+Global.localchegada).global_position
	Global.ondeir(loca1)
	loca2 = get_node("Camera2D/escola/floor/locais/"+Global.localpartida).global_position
	Global.ondeesta(loca2)
	if Global.show1 and Global.show2:
		show.visible = true
